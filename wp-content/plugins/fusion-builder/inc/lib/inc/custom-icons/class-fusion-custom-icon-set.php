	...
	public function add_duplicate_prefix_notice() {
		...
		if ( isset( $icon_set['css_prefix'] ) && true === $this->is_duplicate_prefix( $icon_set['css_prefix'] ) && class_exists( 'Fusion_Admin_Notice' ) ) {
			new Fusion_Admin_Notice(
				...
				'<h2>' . esc_html__( 'Icon set with same CSS prefix already exists!', 'fusion-builder' ) . '</h2><p>' . esc_html__( 'Icon set with same CSS prefix already exists! Please use unique prefix in order to avoid conflicts.', 'fusion-builder' ) . '</p>',
				true,
				'error',
				true,
				'user_meta',
