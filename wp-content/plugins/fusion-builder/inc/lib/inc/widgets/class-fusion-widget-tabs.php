...
class Fusion_Widget_Tabs extends WP_Widget {
	...
	public function __construct() {
		...
		$widget_ops  = [
			...
			'description' => __( 'Popular posts, recent post and comments.', 'fusion-builder' ),
		];
		$control_ops = [
			'id_base' => 'pyre_tabs-widget',
		];

		parent::__construct( 'pyre_tabs-widget', __( 'Avada: Tabs', 'fusion-builder' ), $widget_ops, $control_ops );
			...
			<nav class="fusion-tabs-nav">
				<ul class="tabset tabs">
					...
					<?php if ( $show_popular_posts ) : ?>
						<li class="active"><a href="#" data-link="fusion-tab-popular"><?php esc_html_e( 'Popular', 'fusion-builder' ); ?></a></li>
					<?php endif; ?>

					<?php if ( $show_recent_posts ) : ?>
						<li<?php echo ( ! $show_popular_posts ) ? ' class="active"' : ''; ?>><a href="#" data-link="fusion-tab-recent"><?php esc_html_e( 'Recent', 'fusion-builder' ); ?></a></li>
					<?php endif; ?>

					<?php if ( $show_comments ) : ?>
						<li<?php echo ( ! $show_popular_posts && ! $show_recent_posts ) ? ' class="active"' : ''; ?>><a href="#" data-link="fusion-tab-comments"><span class="awb-icon-bubbles"></span><span class="screen-reader-text"><?php esc_html_e( 'Comments', 'fusion-builder' ); ?></span></a></li>
					<?php endif; ?>
				...
				<?php if ( $show_popular_posts ) : ?>
					...
					<div class="fusion-tab-popular fusion-tab-content tab tab_content" data-name="fusion-tab-popular">
						...
						<ul class="fusion-tabs-widget-items news-list">
							...
							<?php else : ?>
								<li><?php esc_html_e( 'No posts have been published yet.', 'fusion-builder' ); ?></li>
							<?php endif; ?>
						</ul>
					</div>
								...
								<?php foreach ( $the_comments as $comment ) : ?>
									<li>
										...
										<div class="post-holder">
											...
											<p><?php printf( esc_html__( '%s says:', 'fusion-builder' ), esc_html( strip_tags( $comment->comment_author ) ) ); // phpcs:ignore WordPress.WP.AlternativeFunctions ?></p>
											<div class="fusion-meta">
												<?php /* translators: %1$s: comment author. %2$s: post-title. */ ?>
												<a class="comment-text-side" href="<?php echo esc_url( get_comment_link( $comment ) ); ?>" title="<?php printf( esc_attr__( '%1$s on %2$s', 'fusion-builder' ), esc_attr( strip_tags( $comment->comment_author ) ), get_the_title( $comment->comment_post_ID ) ); ?>"><?php echo wp_trim_words( strip_tags( $comment->comment_content ), 12 ); // phpcs:ignore WordPress.Security.EscapeOutput, WordPress.WP.AlternativeFunctions ?></a>
											</div>
										</div>
									</li>
								<?php endforeach; ?>
							<?php else : ?>
								<li><?php esc_html_e( 'No comments have been published yet.', 'fusion-builder' ); ?></li>
							<?php endif; ?>
						</ul>
					</div>
	...
	public function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'design_tabs' ) ); ?>"><?php esc_html_e( 'Tabs Design:', 'fusion-builder' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'design_tabs' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'design_tabs' ) ); ?>" class="widefat" style="width:100%;">
				<option value="classic" <?php echo ( 'classic' === $instance['design_tabs'] ) ? 'selected="selected"' : ''; ?>><?php esc_html_e( 'Classic', 'fusion-builder' ); ?></option>
				<option value="clean" <?php echo ( 'clean' === $instance['design_tabs'] ) ? 'selected="selected"' : ''; ?>><?php esc_html_e( 'Clean', 'fusion-builder' ); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'design_posts' ) ); ?>"><?php esc_html_e( 'Post Design:', 'fusion-builder' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'design_posts' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'design_posts' ) ); ?>" class="widefat" style="width:100%;">
				<option value="image_default" <?php echo ( 'image_default' === $instance['design_posts'] ) ? 'selected="selected"' : ''; ?>><?php esc_html_e( 'Featured Image / Avatar Default Shape', 'fusion-builder' ); ?></option>
				<option value="image_square" <?php echo ( 'image_square' === $instance['design_posts'] ) ? 'selected="selected"' : ''; ?>><?php esc_html_e( 'Featured Image / Avatar Square', 'fusion-builder' ); ?></option>
				<option value="image_circle" <?php echo ( 'image_circle' === $instance['design_posts'] ) ? 'selected="selected"' : ''; ?>><?php esc_html_e( 'Featured Image / Avatar Circle', 'fusion-builder' ); ?></option>
				<option value="post_date" <?php echo ( 'post_date' === $instance['design_posts'] ) ? 'selected="selected"' : ''; ?>><?php esc_html_e( 'Post Date', 'fusion-builder' ); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ); ?>"><?php esc_html_e( 'Popular Posts Order By:', 'fusion-builder' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'orderby' ) ); ?>" class="widefat" style="width:100%;">
				<option value="comment_count" <?php echo ( 'comment_count' === $instance['orderby'] ) ? 'selected="selected"' : ''; ?>><?php esc_html_e( 'Highest Comments', 'fusion-builder' ); ?></option>
				<option value="view_count" <?php echo ( 'view_count' === $instance['orderby'] ) ? 'selected="selected"' : ''; ?>><?php esc_html_e( 'Highest Views', 'fusion-builder' ); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'posts' ) ); ?>"><?php esc_html_e( 'Number of popular posts:', 'fusion-builder' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'tags' ) ); ?>"><?php esc_html_e( 'Number of recent posts:', 'fusion-builder' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'comments' ) ); ?>"><?php esc_html_e( 'Number of comments:', 'fusion-builder' ); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_popular_posts' ) ); ?>"><?php esc_html_e( 'Show popular posts', 'fusion-builder' ); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_recent_posts' ) ); ?>"><?php esc_html_e( 'Show recent posts', 'fusion-builder' ); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_comments' ) ); ?>"><?php esc_html_e( 'Show comments', 'fusion-builder' ); ?></label>
		</p>
		<?php
