...
class Fusion_Widget_Recent_Works extends WP_Widget {
	...
	public function __construct() {
		...
		$widget_ops  = [
			...
			'description' => __( 'Recent works from the portfolio.', 'fusion-builder' ),
		];
		$control_ops = [
			'id_base' => 'recent_works-widget',
		];

		parent::__construct( 'recent_works-widget', __( 'Avada: Recent Works', 'fusion-builder' ), $widget_ops, $control_ops );
	...
	public function form( $instance ) {
		...
		$defaults = [
			'title'  => __( 'Recent Works', 'fusion-builder' ),
			'number' => 6,
		];
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number Of Items To Show:', 'fusion-builder' ); ?></label>
