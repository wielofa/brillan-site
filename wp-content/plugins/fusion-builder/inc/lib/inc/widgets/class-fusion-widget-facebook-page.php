...
class Fusion_Widget_Facebook_Page extends WP_Widget {
	...
	public function __construct() {
		...
		$widget_ops  = [
			...
			'description' => __( 'Adds support for Facebook Page Plugin.', 'fusion-builder' ),
		];
		$control_ops = [
			'id_base' => 'facebook-like-widget',
		];

		parent::__construct( 'facebook-like-widget', __( 'Avada: Facebook Page Plugin', 'fusion-builder' ), $widget_ops, $control_ops );
		...
		<?php if ( $page_url ) : ?>
			...
			<?php endif; ?>
					...
					window.fbAsyncInit = function() {
						...
						function fusion_resize_page_widget() {
							...
							if ( availableSpace != lastAvailableSPace && availableSpace != maxWidth ) {
								...
								jQuery('.<?php echo esc_attr( $args['widget_id'] ); ?> .fb-page' ).attr( 'data-width', Math.floor( availableSpace ) );
					...
					( function( d, s, id ) {
						...
						js.id  = id;
						js.src = "https://connect.facebook.net/<?php echo esc_attr( $language ); ?>/sdk.js#xfbml=1&version=v8.0&appId=<?php echo esc_attr( $app_id ); ?>";
	...
	public function form( $instance ) {
		...
		$defaults = [
			'title'         => __( 'Find us on Facebook', 'fusion-builder' ),
			'page_url'      => '',
			'app_id'        => '',
			'width'         => '268',
			'show_faces'    => 'on',
		...
		<h4 style="line-height: 1.6em;"><?php esc_html_e( 'IMPORTANT: Please create a Facebook App and use its ID for features like sharing.', 'fusion-builder' ); ?> <a href="https://developers.facebook.com/docs/apps/register" target="_blank" rel="noopener noreferrer"><?php esc_attr_e( 'See Instructions.', 'fusion-builder' ); ?></a></h4>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'page_url' ) ); ?>"><?php esc_html_e( 'Facebook Page URL:', 'fusion-builder' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'app_id' ) ); ?>"><?php esc_html_e( 'Facebook App ID:', 'fusion-builder' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'width' ) ); ?>"><?php esc_html_e( 'Width (has to be between 180 and 500):', 'fusion-builder' ); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_faces' ) ); ?>"><?php esc_html_e( 'Show Friends Faces', 'fusion-builder' ); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_stream' ) ); ?>"><?php esc_html_e( 'Show Timeline Tab', 'fusion-builder' ); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_events' ) ); ?>"><?php esc_html_e( 'Show Events Tab', 'fusion-builder' ); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_messages' ) ); ?>"><?php esc_html_e( 'Show Messages Tab', 'fusion-builder' ); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_header' ) ); ?>"><?php esc_html_e( 'Show Cover Photo', 'fusion-builder' ); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'small_header' ) ); ?>"><?php esc_html_e( 'Use Small Header', 'fusion-builder' ); ?></label>
		</p>
		<?php
	}
}
