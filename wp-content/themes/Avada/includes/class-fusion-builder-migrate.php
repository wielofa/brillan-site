	...
	private function get_migration_page_current_action_desc() {
		...
		<?php if ( ! isset( $_GET['step'] ) ) : ?>
			...
			<?php else : ?>
				...
				<p><?php printf( esc_attr__( 'Once reversion is done, you can load a previous version of Avada onto your server. You can download %s here if you do not have a backup of your previous theme.', 'Avada' ), '<a href="https://avada.com/documentation/updating-avada-older-versions/">' . esc_attr__( 'Avada 4.0.3', 'Avada' ) . '</a>' ); ?></p>
		...
		<?php elseif ( isset( $_GET['step'] ) && 'done' === $_GET['step'] ) : ?>
			...
			<?php else : ?>
				...
				<?php printf( esc_attr__( 'All needed posts have been reverted to the previous syntax. You can now roll-back to %s.', 'Avada' ), '<a href="https://avada.com/documentation/updating-avada-older-versions/" target="_blank">' . esc_attr__( 'Avada 4.0.3', 'Avada' ) . '</a>' ); ?>
			<?php endif; ?>
		<?php endif; ?>
		<?php
	}
		...
		<?php elseif ( isset( $_GET['step'] ) && 'done' === $_GET['step'] ) : ?>
			...
			<?php else : ?>
				...
				if ( class_exists( 'FusionCore_Plugin' ) ) {
					$fusion_core_latest = version_compare( FUSION_CORE_VERSION, $bundled_plugins['fusion-core']['version'], '>=' );
				}
