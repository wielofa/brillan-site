...
if ( ! function_exists( 'avada_number_of_featured_images' ) ) {
	 ...
	 * @param bool $count_all Flag to decide if images should be counted even if not displayed.
	 * @return int
	 */
	function avada_number_of_featured_images( $count_all = false ) {
		global $post;
		$number_of_images = 0;

		if ( has_post_thumbnail() && ( $count_all || fusion_get_option( 'show_first_featured_image' ) ) ) {
			$number_of_images++;
		}
