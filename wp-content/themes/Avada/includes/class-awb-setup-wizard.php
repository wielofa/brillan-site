...
class AWB_Setup_Wizard {
	...
	public function __construct() {
		// Fetches and saves color scheme, typography and maintenance globals.
		add_action( 'wp_ajax_awb_save_globals', [ $this, 'ajax_save_globals' ] );

		// Fetches and saves maintenance globals.
		add_action( 'wp_ajax_awb_save_globals_maintenance', [ $this, 'ajax_save_globals_maintenance' ] );
		...
		add_filter( 'upload_mimes', [ Avada()->images, 'allow_svg' ], 100 );
	...
	/**
	 * Save color, typography and maintenance global options.
	 *
	 * @access public
	 * @since 7.7
	 */
	public function ajax_save_globals() {
		...
		$maintenance = isset( $_POST['maintenance'] ) ? wp_unslash( $_POST['maintenance'] ) : 'false'; // phpcs:ignore WordPress.Security.NonceVerification, WordPress.Security.ValidatedSanitizedInput
		...
		$sizes['h5_typography'] = ceil( $sizes['h6_typography'] * $ratio );
		$sizes['h4_typography'] = ceil( $sizes['h5_typography'] * $ratio );
		$sizes['h3_typography'] = ceil( $sizes['h4_typography'] * $ratio );
		$sizes['h2_typography'] = ceil( $sizes['h3_typography'] * $ratio );
		$sizes['h1_typography'] = ceil( $sizes['h2_typography'] * $ratio );
		$small_size             = ceil( 0.8125 * $base );
		$subheading_size        = ceil( 1.5 * $base );
			...
			foreach ( $data as $param => $value ) {
				...
				if ( 'font-size' === $set_parameter ) {
					...
					if ( 'subheadings' === $subset ) {
						$set_value = $subheading_size . 'px';
					}

					if ( 'small' === $subset ) {
						$set_value = $small_size . 'px';
					}
				}
		...
		if ( 'true' === $maintenance ) {
			$saved_options['maintenance_mode']       = 'coming_soon';
			$saved_options['maintenance_template']   = '0';
			$saved_options['maintenance_user_roles'] = [ 'administrator' ];
		}
	...
	/**
	 * Save maintenance global options.
	 *
	 * @access public
	 * @since 7.9
	 * @return void
	 */
	public function ajax_save_globals_maintenance() {
		$this->check_nonce();
			...
			<header class="avada-db-header-main">
				<div class="avada-db-header-main-container">
					<a class="avada-db-logo" href="<?php echo esc_url( admin_url( 'admin.php?page=avada' ) ); ?>" aria-label="<?php esc_attr_e( 'Link to Avada dashboard', 'Avada' ); ?>">
						...
						<div class="heading"><?php esc_html_e( 'Setup Wizard', 'Avada' ); ?></div>
					</a>
					<div class="wizard-hero-header">
						<a class="button button-primary" target="_blank" href="https://avada.com/documentation/how-to-use-the-avada-setup-wizard/"><?php esc_html_e( 'Documentation', 'Avada' ); ?></a>
					</div>
				</div>
			</header>
