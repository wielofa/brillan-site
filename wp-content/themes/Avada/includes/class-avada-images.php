...
class Avada_Images extends Fusion_Images {
	 ...
	 * @return void
	 */
	public function render_placeholder_image( $featured_image_size = 'full', $aspect_ratio = '' ) {
		...
		?>
			<div class="fusion-placeholder-image" data-origheight="<?php echo esc_attr( $height ); ?>" data-origwidth="<?php echo esc_attr( $width ); ?>" style="<?php echo esc_attr( $style ); ?>"></div>
		<?php
	}
	...
	public function get_image_orientation_class( $attachment ) {
		...
		$orientation_class     = 'fusion-image-grid';
		...
		if ( $attachment[1] / $attachment[2] > $sixteen_to_nine_ratio ) {
			$orientation_class = 'fusion-image-landscape';
		}

		// Portrait.
		if ( $attachment[2] / $attachment[1] > $sixteen_to_nine_ratio ) {
			$orientation_class = 'fusion-image-portrait';
		}
